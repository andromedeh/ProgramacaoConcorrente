...
* Nome.............: ThreadTrem1
* Funcao...........: Controla a thread do trem 1 e as solucoes para a nao-colisao, 
...
public class ThreadTrem1 extends Thread {
  ...
  private int solucao; // tbm escolhida pelo usuario na interface
  private int processo = 0; // variavel usada na solucao de peterson
  private boolean isPaused = false; // flag necessaria em metodo de pausar a thread

  public ThreadTrem1 (TelaPrincipalController controlador) {
    this.controlador = controlador;
    this.trem = controlador.getIconTrem1();
  ...
  public void solucaoVariavelTravamento() throws InterruptedException{
    switch(sentidoDoTrem){
      case 0:{ // trem superior
        ...
        controlador.alterarVT1();// avisa que vai entrar em RC1/tunel1
        descer(175); // movimenta no tunel 1
        controlador.alterarVT1(); // avisa que saiu do RC1/tunel 1
        descer(226); // regiao nao critica
        verificaRC2(); // verifica VT2 e para o trem se preciso
        controlador.alterarVT2();// avisa que vai entrar em RC2/tunel2
        descer(365); // movimenta no tunel 2
        controlador.alterarVT2(); // avisa que saiu de RC2
        descer(460); // regiao nao critica
        posicaoY = -18;
        Platform.runLater( () -> trem.setLayoutY(-18)); // volta pro comeco
        break;
        } 
  ...
  public void solucaoEstritaAlternancia() throws InterruptedException{
    switch(sentidoDoTrem){
      case 0:{ // trem descendo
        ...
        descer(175); // movimenta no tunel 1
        controlador.passarVEZ1(1); // passa a vez quando sai
        descer(226); // desce ate perto do tunel 2
        esperaVEZ2(); // espera vez no tunel 2
        descer(365); // movimenta no tunel 2
        controlador.passarVEZ2(1); // passa a vez quando sai
      ...
      case 1:{ // trem subindo
        ...
        esperaVEZ2(); // espera vez no tunel 2
        subir(226); // movimenta no tunel 2
        controlador.passarVEZ2(1); // passa a vez quando sai 
  ...
  public void solucaoPeterson() throws InterruptedException{
    switch(sentidoDoTrem){
      case 0:{ // trem descendo
        trem.setLayoutY(-18); // seta posicao inicial
        descer(26); // desce ate perto do tunel 1
        entrarTunel1(); // metodo da solucao de peterson
        descer(175); // movimenta no tunel 1
        sairTunel1();
        descer(226); // desce ate perto do tunel 2
        entrarTunel2();
      ...
      case 1:{ // trem subindo
        trem.setLayoutY(410); // seta posicao inicial
        subir(380); // sobe ate perto do tunel 2
        entrarTunel2(); // metodo da solucao de peterson
        subir(226); // movimenta no tunel 2
        sairTunel2();
        subir(190); // sobe ate perto do tunel 1
        entrarTunel1();
  ...
  public void esperaVEZ1(){
    while(controlador.getVEZ1()!=0){ // espera sua vez no tunel 1
      try {
        if (controlador.isReseted()){ // garantir que ele resete a posicao quando estiver em sleep
        controlador.setReseted(false);
        break;
  ...
  public void esperaVEZ2(){
    while(controlador.getVEZ2()!=0){ // espera sua vez no tunel 2
      try {
        if (controlador.isReseted()){ // garantir que ele resete a posicao quando estiver em sleep
          controlador.setReseted(false);
          break;
